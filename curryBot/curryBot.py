...
CONFIG = {
    'Doubt': {
        'commands' : ['X', 'x'],
        'messageRegex': r'[Xx]$',
        'packs' : ['user_269942836_by_plakbot'],
        'amount' : 5,
        'transitiveReply': True
    },

    'Eelco': {
        'messageRegex': r'.*[Ee]elco',
        'packs' : ['eelco'],
        'amount' : 1,
        'transitiveReply': False
    },
    
    'Hmmm': {
        'messageRegex': r'[Hh][mM]+(\s.*)?$',
        'packs' : ['thonkang'],
        'amount' : 1,
        'transitiveReply': True
    },
...
def start_bot(token):
    ...
    for botConfig in CONFIG:
        print('Initializing bot %s' % botConfig)
        init_bot(botConfig, updater)
